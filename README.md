...
Protenix:
    ...
    python ipsae.py <path_to_protenix_full_data_json_file> <path_to_protenix_cif_file> <pae_cutoff> <dist_cutoff>
    python ipsae.py il12_full_data_sample_0.json il12_sample_0.cif 10 15

    IPSAE accepts Protenix full-data JSON files that contain token-level PAE values in `token_pair_pae`
    and atom-level confidence values in `atom_plddt`. The matching summary file is resolved automatically
    from the sibling `*_summary_confidence_*.json` file when present.
...
Esmfold2:
    ...
    python ipsae.py <path_to_Esmfold2_pae_npy_file> <path_to_Esmfold2_cif_file> <pae_cutoff> <dist_cutoff>
    python ipsae.py model_0.npy model_0.cif 10 10
