...
# includes support for Boltz structures, Protenix full_data outputs, and structures with nucleic acids
...
#  python ipsae.py <path_to_protenix_full_data>     <path_to_protenix_cif_file>  <pae_cutoff> <dist_cutoff>
#
...
if len(sys.argv) < 5:
    ...
    print("")
    print("Usage for Protenix (mmCIF format):")
    print("   python ipsae.py <path_to_full_data_json_file> <path_to_mmcif_file> <pae_cutoff> <dist_cutoff>")
    print("   python ipsae.py il12_full_data_sample_0.json il12_sample_0.cif 10 15")
    print("")
...
def resolve_summary_file_path(pae_file_path):
    candidates = []

    for old, new in [
        ("confidences", "summary_confidences"),
        ("full_data", "summary_confidences"),
        ("confidences", "summary_confidence"),
        ("full_data", "summary_confidence"),
    ]:
        if old in pae_file_path:
            candidate = pae_file_path.replace(old, new)
...
if af3:
    ...
    #   model1.cif
    # Example Protenix filenames
    #   il12_full_data_sample_0.json
    #   il12_summary_confidence_sample_0.json
    #   il12_sample_0.cif
    if os.path.exists(pae_file_path):
        with open(pae_file_path, 'r') as file:
            data = json.load(file)
    ...
    elif "atom_plddt" in data:
        atom_plddts = np.array(data['atom_plddt'])
    else:
        atom_plddts = None

    if atom_plddts is not None:
        if np.max(atom_plddts) <= 1.0:
    ...
    if 'pae' in data:
        ...
        pae_matrix_af3 = np.array(data['token_pair_pae'])
    else:
        print("no PAE data in AF3/Protenix json file; quitting")
        sys.exit()

    # AF3 full_data keeps an atom-token matrix that needs residue-token masking.
    # Protenix full_data already stores a token-level matrix.
    token_array = np.array(token_mask)
    if pae_matrix_af3.shape[0] == len(token_array):
        pae_matrix = pae_matrix_af3[np.ix_(token_array.astype(bool), token_array.astype(bool))]
    elif pae_matrix_af3.shape[0] == int(np.sum(token_array)):
        pae_matrix = pae_matrix_af3
    else:
        print(f"Unexpected AF3/Protenix PAE shape {pae_matrix_af3.shape}; token mask length {len(token_array)} and selected token count {int(np.sum(token_array))}")
        sys.exit()

    # Get iptm matrix from AF3/Protenix summary confidence file
    iptm_af3 = {chain1: {chain2: 0 for chain2 in unique_chains if chain1 != chain2} for chain1 in unique_chains}

    summary_file_path = resolve_summary_file_path(pae_file_path)
